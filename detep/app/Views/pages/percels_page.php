...
<div class="card mb-1">
    <li
        ...
        <div>
            ...
            <?= $row['parc_sold'] == 1 ? '<span data-feather="check-circle"></span>' : '' ?>
        ...
        <div class="responsive">
            ...
            <div class="btn-group btn-group-sm mr-2" role="group" aria-label="First group">
                <?php 
                $i = 0;
                $numItems = count($parcel_path);
                $showArrow = true;
                foreach ($parcel_path as $key=>$value) : 
                    $sentKey = $key;
                    ++$i;
                     if($i === $numItems)
                     {
                ...
                <button type="button"
                    ...
                    data-one='<?= $arrivals[$key] ?>' id='<?= $value ?>'><?= 'S' . $i ?>
                    <?= $showArrow ? '<span data-feather="chevrons-right"></span>' : '' ?>
                    </button>
                <?php endforeach ?>
            </div>
            <?php endif ?>
            ...
            <div class="btn-group btn-group-sm" role="group" aria-label="Second group">
                <button type="button"
                    ...
                    data-one="<?= $last_sent_date ?>" id="<?= $next ?>"><?= 'W' ?></button>
            </div>
            <?php endif ?>
        </div>
...
<div class="card mb-1">
    <li
        ...
        <div>
            ...
            <?= $row2['parc_sold'] == 1 ? '<span data-feather="check-circle"></span>' : '' ?>
            <span class="badge badge-info ml-2"><?= esc($row['sy_title']) ?></span>
        </div>
...
<li
    ...
    if($row2['parc_station_path'] !== NULL) :
        ...
        <div class="btn-group btn-group-sm mr-2" role="group" aria-label="First group">
            ...
            <button type="button"
                class="btn <?= session()->get('int_station') == $value ? 'btn-success' : 'btn-secondary' ?> hover"
                data-one='<?= $arrivals[$key] ?>' id='<?= $value ?>'>
                <?= 'S' . $i ?>
                <?= $showArrow ? '<span data-feather="chevrons-right"></span>' : '' ?></button>
            <?php endforeach ?>
        </div>
        <?php endif ?>
        <?php  if($row2['parc_next_station'] !== NULL) :
